-- Use the existing stripe_charges table and enhance it, then create the view
-- First, let's enhance the existing stripe_charges table to be more useful
DROP TABLE IF EXISTS public.stripe_charges;

-- Create a better stripe_charges table based on the existing foreign table
...
CREATE POLICY "Application payment status readable by authenticated users" ON public.application_payment_status
  FOR SELECT USING (auth.role() = 'authenticated');
...
ON public.stripe_checkout_sessions USING GIN ((metadata->>'requestId'));
