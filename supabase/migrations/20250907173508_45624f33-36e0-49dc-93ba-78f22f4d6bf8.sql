...
DECLARE
  v_api_key_id text;
BEGIN
  -- Get the secret key ID from vault
  SELECT id INTO v_api_key_id FROM vault.secrets WHERE name = 'STRIPE_SECRET_KEY' LIMIT 1;
  
  IF v_api_key_id IS NULL THEN
    RAISE EXCEPTION 'STRIPE_SECRET_KEY not found in vault.secrets';
  END IF;
  
  -- Create the server
  EXECUTE format(
    'CREATE SERVER IF NOT EXISTS stripe_server FOREIGN DATA WRAPPER stripe_wrapper OPTIONS (api_key_id ''%s'', api_url ''https://api.stripe.com/v1'')',
    v_api_key_id
  );
END $$;
