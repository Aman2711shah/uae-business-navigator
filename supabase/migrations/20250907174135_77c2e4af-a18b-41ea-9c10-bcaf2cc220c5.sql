...
-- Create Stripe tables for tracking payment data
...
-- Create indexes for better performance (fixed GIN index)
...
-- Create a functional index for requestId lookup
CREATE INDEX IF NOT EXISTS idx_stripe_checkout_sessions_request_id 
ON public.stripe_checkout_sessions ((metadata->>'requestId'));