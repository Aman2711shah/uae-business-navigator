        ...
        {navItems.map((item, index) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center py-3 px-4 rounded-xl transition-all duration-300 transform hover:scale-105 ${
                isActive
                  ? "text-primary bg-primary/15 shadow-sm scale-105"
                  : "text-muted-foreground hover:text-foreground hover:bg-muted/60"
              }`}
            >
              <item.icon className={`h-6 w-6 mb-1 transition-all duration-200 ${isActive ? 'stroke-2' : 'stroke-1.5'}`} />
